...
class CoinDetailViewModel @Inject constructor(
    ...
    private fun getCoin(coinId: String) {
        getCoinUseCase(coinId).onEach { results ->
            when(results){
                is Resource.Success -> {
                    _state.value = CoinDetailState(coin = results.data)
                }
                is Resource.Error ->{
                    _state.value = CoinDetailState(error = results.message ?: "An unexpected Error occurred")
                }
                is Resource.Loading ->{
